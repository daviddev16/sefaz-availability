...
import com.daviddev16.core.PlaceholderResolver;
import com.daviddev16.core.Placeholders;
...
public final class MainApplication {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String bannerCardJson = read(getResourceAsStream("/banner_card.json"));
			Config.set(Config.INTERNAL_BANNER_CARD_CONTENT, bannerCardJson);

			LOG.info("Carregando card de template interno.");
			String baseCardJson = read(getResourceAsStream("/base_card.json"));
			Config.set(Config.INTERNAL_BASE_CARD_CONTENT, baseCardJson);

			String errorCardJson = read(getResourceAsStream("/error_card.json"));
			Config.set(Config.INTERNAL_ERROR_CARD_CONTENT, errorCardJson);
			...
			while (true) {
				...
				for (NFModality nfModality : NFModality.values()) {
					...
					try {
						...
						String errorJson = new PlaceholderResolver(errorCardJson)
								.assign(Placeholders.EXCEPTION_MESSAGE, exceptionMessage)
								.resolved();
						
						((IntegrationStatusObserver)observer).getGoogleChatSpace().sendJson(errorJson);
